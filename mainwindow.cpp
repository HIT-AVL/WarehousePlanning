...
void MainWindow::on_initHouseButton_clicked()//³õÊ¼»¯²Ö¿â
{
    QVector<int > killer;
...
void MainWindow::on_initInputButton_clicked()//Èë¿â³õÊ¼»¯±íÏî
{
    ui->inputTable->setColumnCount(0);
    ui->inputTable->setRowCount(0);//ĞĞÊıÁĞÊıÇåÁã

    ui->inputTable->setColumnCount(8);
    ui->inputTable->setRowCount(18);//³õÊ¼»¯Îª8ÁĞ18ĞĞ
    QStringList headers;
    headers<<"Name"<<"Category"<<"Amount"<<"Shelf ID"<<"Direction"<<"Colum"<<"Layer"<<"Description";//»õÎïĞÅÏ¢ÏîÄ¿
    ui->inputTable->setHorizontalHeaderLabels(headers);//Î»ÖÃÉè¶¨
    for (int i=0;i<18;i++){
        for (int j=0;j<8;j++)
...
void MainWindow::on_finInputButton_clicked()//±íÏî»õÎïÈë¿â
{

    for (int i=0;i<ui->inputTable->rowCount();i++)//½«±íÖĞĞÅÏ¢Èë¿â
    ...
    ui->inputTable->setColumnCount(0);
    ui->inputTable->setRowCount(0);//½«±íÇå¿Õ
}
#include <QDebug>
void MainWindow::on_massInputButton_clicked()//ÅúÁ¿Èë¿â
{
    QString filename = QFileDialog::getOpenFileName();//È¡µÃÎÄ¼şÃû³Æ
    QFileInfo info(filename);//È¡µÃÎÄ¼şÂ·¾¶
    if (info.exists() && info.isFile()) {//ÅĞ¶ÏÎÄ¼şÊÇ·ñ´æÔÚ
        int m;//¼ÇÂ¼ÎÄ¼şÖĞµÄ±íµ¥ÊıÄ¿
        FILE *fp=fopen(filename.toStdString().c_str(),"r");//ÒÔÖ»¶Á·½Ê½´ò¿ªÎÄ¼ş
        fscanf(fp,"%d\n",&m);//¶Á³ö±íµ¥ÊıÄ¿
        //qDebug()<<"m="<<m;
        storeitem tmp;
        char name[200],category[200],description[200];
        int num,belong_to,x,y,z;//num-»õÎïÊıÁ¿£¬belong_to-Êé¼ÜµÄID£¬x-·½Ïò£¬y-ÁĞ£¬z-²ã
        for (int i=0;i<m;i++){
            fscanf(fp,"%[^\t]\t%[^\t]\t%d%d%d%d%d\t%[^\n]\n",name,category,&num,&belong_to,&x,&y,&z,description);//°´ÕÕ¹Ì¶¨¸ñÊ½¶ÁÈ¡ÎÄ¼şÖĞµÄÊı¾İ
            ...
            item->insert_item(tmp);//±íµ¥Èë¿â²Ù×÷
        }
        fclose(fp);
    }
	else{
		 QMessageBox::information(NULL, "´íÎó", "ÕÒ²»µ½ÎÄ¼ş", QMessageBox::Yes);//´íÎóÌáÊ¾
	}
}

void MainWindow::on_initOutputButton_clicked()//³ö¿â³õÊ¼»¯±íÏî
{
    ui->outputTable->setColumnCount(0);
    ui->outputTable->setRowCount(0);//ĞĞÊıÁĞÊıÇåÁã

    ui->outputTable->setColumnCount(3);
    ui->outputTable->setRowCount(14);//³õÊ¼»¯Îª3ÁĞ14ĞĞ
    QStringList headers;
    headers<<"Name"<<"Category"<<"Amount";//³ö¿â»õÎïĞÅÏ¢
    ui->outputTable->setHorizontalHeaderLabels(headers);
    for (int i=0;i<14;i++){
        for (int j=0;j<8;j++)
            ui->outputTable->setItem(i,j,new QTableWidgetItem(QString()));//³õÊ¼»¯±íÏî
    }
}

void MainWindow::on_pathButton_clicked()//Ñ°ÕÒ×î¶ÌÂ·¾¶
{
    QVector<QPair<QPair<QString,QString>,int> > keyword;//ÒÔÃû³Æ£¬ÖÖÀà£¬Êé¼ÜºÅÈ·¶¨Ò»±¾Êé
    ...
    for (int i=0;i<(int)ui->outputTable->rowCount();i++){
        ...
        QString num     =ui->outputTable->item(i,2)->text();//È¡µÃµÚiĞĞ±íµ¥µÄÊı¾İ
        if (name.size()==0||category.size()==0||num.size()==0){//Èç¹û±íµ¥Îª¿Õ£¬Ìø¹ı²åÈë¹ı³Ì
            continue;
        }
        QPair<QPair<QString,QString>,int> tmp=qMakePair(qMakePair(name,category),num.toInt());
        //qDebug()<<i<<","<<name<<","<<category<<num;
        keyword.push_back(tmp);//½«ĞÂ±íµ¥µÄĞÅÏ¢Á¬ÔÚÏòÁ¿Î²²¿
    }
    //qDebug()<<scene->beg->pos().x()<<scene->beg->pos().y();
    bill->find_path(shelf,item,keyword,scene->beg->x(),scene->beg->y());//¼ÆËãÂ·¾¶
...
void MainWindow::on_finOutputButton_clicked()//±íÏî»õÎï³ö¿â
{
    ui->outputTable->setColumnCount(0);
    ui->outputTable->setRowCount(0);//½«±íÇå¿Õ
    bill->make_deal(shelf,item);//Ïû³ıÂ·¾¶
    return ;
}

void MainWindow::on_massOutputButton_clicked()//ÅúÁ¿³ö¿â
{
...
void MainWindow::on_searchButton_clicked()//¼ìË÷¹¦ÄÜ
{
    ui->searchTable->setColumnCount(0);
    ui->searchTable->setRowCount(0);//±íÏîÇå¿Õ
...
void MainWindow::get_items_of_shelf(int id){//ÏÔÊ¾Êé¼ÜĞÅÏ¢
    ui->searchTable->setColumnCount(0);
...
void MainWindow::on_pushButton_3_clicked()
{
    ...
    if(f.exists())
    {
        ...
        msgBox.setInformativeText("æ˜¯å¦è¦æ›¿æ¢å·²æœ‰çš„æ•°æ®åº“ä¿¡æ¯ï¼Ÿ");
   ...
   // t << MultiLineEdit1->text();
    ...
    {
        QMessageBox::information(this, "æ³¨æ„", "è¯·å…ˆè®¾ç½®ç”¨æˆ·ä¿¡æ¯ï¼Œå†å°è¯•åˆå§‹åŒ–!", QMessageBox::Ok );
        return ;
    }
    char cmd[1000];
...
void MainWindow::on_pushButton_clicked()
{
    ...
    if(f.exists())
    {
        ...
        QMessageBox msgBox;
        msgBox.setStandardButtons(QMessageBox::Yes| QMessageBox::No);
        msgBox.setInformativeText("æ˜¯å¦è¦æ›¿æ¢å·²æœ‰çš„ç”¨æˆ·ä¿¡æ¯ï¼Ÿ");
       // QMessageBox::information(this, "Document", , QMessageBox::Ok | QMessageBox::Cancel);
        switch (msgBox.exec())
        {
            case QMessageBox::No:
...
void MainWindow::on_pushButton_4_clicked()
{
    ...
    if(f)
    {
        QMessageBox::information(0, "æ¶ˆæ¯", "é‡æ–°è¿æ¥æˆåŠŸ!", QMessageBox::Ok );
    }
    else
    {
        QMessageBox::information(0, "æ¶ˆæ¯", "é‡æ–°è¿æ¥å¤±è´¥!", QMessageBox::Ok );
    }
